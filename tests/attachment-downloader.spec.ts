...
describe("attachment downloader", () => {
  it("downloads an https attachment through the browser request context", async () => {
    ...
    expect(path.relative(tempDir, result.path)).toBe(path.join("downloads", "conv-http", "output.csv"));
  ...
  it("adds an extension from the response mime type when a filename has none", async () => {
    ...
    const result = await downloadAttachment(pageWithHttp({
      "https://example.test/generated/image": {
        body: Buffer.from([0x89, 0x50, 0x4e, 0x47]),
        headers: { "content-type": "image/png" },
      },
    }).page, "conv-mime-extension", "user-image-1");
  ...
  it("downloads blob attachments by evaluating fetch in the page", async () => {
    ...
    const result = await downloadAttachment(pageWithBlob(new Uint8Array([1, 2, 3])), "conv-blob", "image-1");
  ...
  it("throws a typed error for a missing id", async () => {
    ...
    await expect(downloadAttachment(emptyPage(), "conv-missing", "file-404"))
      .rejects
      .toMatchObject({
  ...
  it("continues downloadAll after per-item failures", async () => {
    ...
    const results = await downloadAll(pageWithHttp({
  ...
  it("throws from downloadAll only when every selected attachment fails", async () => {
    ...
    await expect(downloadAll(pageWithHttp({}).page, "conv-all-fail"))
      .rejects
      .toBeInstanceOf(AttachmentDownloadError);
  });
...
type HttpResponse = Buffer | {
  body: Buffer;
  headers?: Record<string, string>;
};

function pageWithHttp(responses: Record<string, HttpResponse>): { page: Page; requests: () => { gets: number; bodies: number } } {
  let getCount = 0;
  let bodyCount = 0;
  const page = {
    context: () => ({
