...
} from "../src/browser/attachments.ts";
...
describe("attachment extraction", () => {
  ...
  it("extracts assistant file download links without changing user messages", async () => {
    const messages = await extractAllMessages(pageWithAll([
      userMessage("upload.csv"),
      assistantMessage([
        text("Result: "),
        el("a", { download: "result.csv", href: "blob:https://chatgpt.test/result" }, [text("result.csv")]),
      ]),
  ...
  it("increments counters independently per role and kind", async () => {
    ...
    expect(messages.flatMap((message) => message.attachments.map((attachment) => attachment.id))).toEqual([
      "user-image-1",
      "image-1",
      "user-image-2",
      "image-2",
  ...
  it("persists counters across extractions in one conversation", async () => {
    const first = await extractAssistantContent(pageWithLast(assistantMessage([
      el("img", { src: "https://example.test/first.png" }),
    ])), { conversationId: "conv-counter" });
    const firstAgain = await extractAssistantContent(pageWithLast(assistantMessage([
      el("img", { src: "https://example.test/first.png" }),
    ])), { conversationId: "conv-counter" });
    const second = await extractAssistantContent(pageWithLast(assistantMessage([
      el("img", { src: "https://example.test/second.png" }),
    ])), { conversationId: "conv-counter" });
...
function el(tagName: string, attributes: Record<string, string> = {}, children: DomSnapshotNode[] = []): DomSnapshotNode {
