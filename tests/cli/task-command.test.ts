...
describe("task command", () => {
  ...
  it("opens connector setup with the normalized connector URL", async () => {
    ...
    ctx.config.tunnelUrl = "https://bridge.example/";
    ctx.orchestrator.openConnectorSetup = async (connectorUrl: string) => {
      setupCalls.push(connectorUrl);
      return {
        connectorUrl,
        completed: false,
  ...
  it("does not run browser setup when no tunnel URL exists", async () => {
    ...
    ctx.orchestrator.openConnectorSetup = async (connectorUrl: string) => {
      setupCalled = true;
      return { connectorUrl, completed: true, steps: [], warnings: [] };
    };
    const log = vi.spyOn(console, "log").mockImplementation(() => {});
  ...
  it("detects natural project requests for MCP-first wrapping", () => {
    ...
    expect(shouldAutoWrapProjectPrompt("refactor @src/cli/app.tsx")).toBe(true);
