 ...
 * a large fan-out cannot flood the caller's context; each reply is truncated to `maxReplyChars`.
 ...
 * @returns True when the fan-out should be treated as failed.
 * @example
 * ```ts
 * const failed = fanoutFailed(result, true);
 * ```
 */
