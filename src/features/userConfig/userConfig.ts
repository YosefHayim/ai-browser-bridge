...
const userConfig = {
   ...
   * @returns The `loadHooks` result.
   * @example
   * ```ts
   * const result = await userConfig.loadHooks(options);
   * ```
   */
   ...
   *
   * @param options - Options that configure the method.
   * @returns The `loadCustomCommands` result.
   * @example
   * ```ts
   * const result = await userConfig.loadCustomCommands(options);
   * ```
   */
   ...
   *
   * @param repoRoot - Repo root value.
   * @returns The `loadProjectInstructions` result.
   * @example
   * ```ts
   * const result = await userConfig.loadProjectInstructions(repoRoot);
   * ```
   */
  ...
  async readHookFile(path: string): Promise<ParseHooksResult | null> {
    let raw: string;
    try {
  ...
  commandDirs(options: LoadCustomCommandsOptions): CommandDir[] {
  ...
  async loadCommandsFromDir(entry: CommandDir): Promise<CustomCommand[]> {
    const commands: CustomCommand[] = [];
  ...
  async loadCommandFile(input: { entry: CommandDir; fileName: string }): Promise<CustomCommand> {
  ...
  async readMarkdownFiles(dir: string): Promise<string[]> {
    try {
 ...
 * @param homeDir - Home dir value.
 * @returns The `hookConfigPaths` result.
 * @example
 * ```ts
 * const result = hookConfigPaths(input, homeDir);
 * ```
 */
 ...
 * @example
 * ```ts
 * const result = isHookLifecycleEvent(value);
 * ```
 */
...
const isHookCommand = (value: unknown): value is string | readonly string[] => {
  return (
    typeof value === "string" ||
 ...
 * @returns The `parseHooksConfig` result.
 * @example
 * ```ts
 * const result = parseHooksConfig(input, source);
 * ```
 */
export const parseHooksConfig = (
...
const parseHookArray = (input: { hooksValue: unknown[]; source: string }): ParseHooksResult => {
  const hooks: HookDefinition[] = [];
  const errors: string[] = [];
...
const parseHookEventHooks = (input: {
  eventName: string;
  value: unknown;
  source: string;
  hooks: HookDefinition[];
...
const validateHookFields = (input: {
 ...
 * @returns The `runHooks` result.
 * @example
 * ```ts
 * const result = await runHooks(input, hooks);
 * ```
 */
...
const mapHookRun = (input: { event: HookLifecycleEvent; hook: HookDefinition }): HookRunResult => {
 ...
 *
 * @param markdown - Markdown value.
 * @returns The `parseCustomCommandFile` result.
 * @example
 * ```ts
 * const result = parseCustomCommandFile(markdown);
 * ```
 */
 ...
 * @returns The `renderCustomCommandPrompt` result.
 * @example
 * ```ts
 * const result = renderCustomCommandPrompt(command, args);
 * ```
 */
...
const applyFrontmatterLine = (input: {
  metadata: CustomCommandMetadata;
  lines: string[];
  index: number;
}): number | undefined => {
...
const applyFrontmatterKey = (input: {
  metadata: CustomCommandMetadata;
  lines: string[];
  index: number;
  key: string;
...
const parseAllowedToolsList = (input: {
  metadata: CustomCommandMetadata;
  lines: string[];
  index: number;
}): number => {
...
const consumeArgumentChar = (input: {
  char: string;
...
const renderTemplateReplacement = (input: {
  match: string;
 ...
 * @returns The `renderProjectInstructions` result.
 * @example
 * ```ts
 * const result = renderProjectInstructions(files);
 * ```
 */
...
/**
 * Load local and user hook configs.
 *
 * @param options - Options that configure the operation.
 * @returns The `loadHooksConfig` result.
 * @example
 * ```ts
 * const result = await loadHooksConfig(options);
 * ```
 */
export const loadHooksConfig = async (options: LoadHooksOptions): Promise<LoadedHooksConfig> => {
  return userConfig.loadHooks(options);
};
 ...
 * @example
 * ```ts
 * const result = await loadCustomCommands(options);
 * ```
 */
export const loadCustomCommands = async (
  options: LoadCustomCommandsOptions,
): Promise<CustomCommand[]> => {
  return userConfig.loadCustomCommands(options);
};
 ...
 * @example
 * ```ts
 * const result = await loadProjectInstructions(repoRoot);
 * ```
 */
export const loadProjectInstructions = async (repoRoot: string): Promise<ProjectInstructions> => {
  return userConfig.loadProjectInstructions(repoRoot);
};
