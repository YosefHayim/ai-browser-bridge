...
//
//   node scripts/dev/captureModelPicker.mjs            # claude, grok, perplexity
//   node scripts/dev/captureModelPicker.mjs claude
//
...
import { PROVIDER_CONFIG } from "../../src/config.ts";
...
const captureOne = async (browser, id) => {
  const providerConfig = PROVIDER_CONFIG[id];
  if (providerConfig === undefined) return { provider: id, error: "unknown provider" };
  const modelTrigger = providerConfig.selectors.modelTrigger;
  if (modelTrigger === undefined) return { provider: id, error: "provider has no model picker" };
  const page = await findPage(browser, providerConfig.origin);
  if (!page) return { provider: id, error: `no ${providerConfig.origin} tab open` };
  const opened = await page
    ...
    .then(() => true)
    .catch(() => false);
  if (!opened) return { provider: id, error: `could not open picker via ${modelTrigger}` };
  await page.waitForTimeout(900);
  const result = await page.evaluate(pickerProbe).catch((err) => ({
    count: 0,
    items: [],
    error: String(err).split("\n")[0],
  }));
  await page.keyboard.press("Escape").catch(() => {});
  return { provider: id, trigger: modelTrigger, ...result };
};
