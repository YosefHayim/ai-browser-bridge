...
//   node scripts/dev/verifyProviders.mjs           # verify the default five
//   node scripts/dev/verifyProviders.mjs grok claude --timeout=90
//
...
const classify = (result) => {
  ...
  if (result.code === 0 && payload) {
    ...
    return {
      status: "EMPTY",
      model: payload.model ?? "",
      detail: "reply captured was empty",
    };
  }
  ...
  if (/timed out|timeout/i.test(reason) || result.code === null) {
    return {
      status: "TIMEOUT",
      model: "",
      detail: reason || "killed by wall-clock guard",
    };
  }
