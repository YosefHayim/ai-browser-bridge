...
//
//   node scripts/dev/captureChatgptSelectors.mjs
//
...
const sidebarProbe = () => {
  ...
  const chats = [...document.querySelectorAll('a[href^="/c/"]')].map((a) => {
    ...
    return {
      ...
      optionsTestid: optByTitle.get(title) ?? "",
    };
  });
  const nav = [
  ...
  return {
    ...
    newProjectButton: newProject
      ? {
          aria: "New project",
          testid: newProject.getAttribute("data-testid") || "",
        }
      : null,
...
const projectsProbe = () => {
  ...
  return {
    ...
    newProjectButton: newProject
      ? {
          testid: newProject.getAttribute("data-testid") || "",
          text: clip(newProject.textContent),
        }
      : null,
  };
};
...
const probeChatMenu = async (page, sidebar) => {
  ...
  try {
    ...
    await page.click(`[data-testid="${target.optionsTestid}"]`, {
      force: true,
      timeout: 8000,
    });
...
try {
  ...
  console.error(
    `Could not attach to Chrome on ${CDP_URL}.\nRun \`node dist/bridge.js chrome start\` (or \`bridge chrome start\`), sign into ChatGPT if needed, leave Chrome open, then re-run: node scripts/dev/captureChatgptSelectors.mjs`,
  );
  process.exit(1);
}
