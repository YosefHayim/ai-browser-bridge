...
// validates the selectors declared in src/config.ts against the
...
import { PROVIDER_CONFIG, PROVIDER_IDS } from "../../src/config.ts";
...
const DEFAULT_PROVIDERS = PROVIDER_IDS.filter((providerId) => providerId !== "chatgpt");
...
const selectorProbe = (configured) => {
  ...
  const countOf = (sel) => {
    if (!domSafe(sel))
      return {
        count: -1,
        note: "playwright :has-text (checked by bridge, not DOM)",
      };
    try {
...
const captureOne = async (browser, id) => {
  const provider = PROVIDER_CONFIG[id];
  ...
  if (id === "flow") await enterFlowEditor(page);
  ...
  if (id === "chatgpt") {
    report.connector = {
      available: true,
      note: "implemented flow — setupMcpConnectorInChatGpt",
    };
